    ...
    <div class="inputs">
      ...
      <div class="numberOfPeople" :style="numberOfPeopleChange && 'border-color:#38f'">
        <div @click.stop="numberOfPeopleChange=!numberOfPeopleChange" style="width:100%">
          <span
            :style="numberOfPeopleChangeOk ? 'color:#606266' : ''"
          >{{numberOfPeopleChangeOk ? numberOfPeopleChangeOk : '人数未定'}}</span>
      ...
      <div class="evaluate">
        <div class="clearfix">
          ...
          <div class="evaluateAll">
            <div
              class="evaluateAllA"
              :style="evaluateAllAOen ? `height: 60px;` : ''"
              v-if="cityScenics.length !== 0"
            >
              <a href="javascrip:;" v-for="(v,i) in cityScenics" :key="i">{{v.name}}</a>
            </div>
            <div @click="evaluateAllAOen=!evaluateAllAOen" style="width:98px">
              <span
                class="el-icon-d-arrow-right evaluateAllOpenDwon"
                :style="evaluateAllAOen ? '' : `transform: rotate(270deg)`"
              ></span>
        ...
        <el-col :span="8">
          <div class="grid-content bg-purple">
            <div class="Slider screenUnified">
              ...
              <el-slider
                v-model="hotelPricekey"
                :format-tooltip="formatTooltip"
                @change="getComplexHotel"
              ></el-slider>
            </div>
          </div>
        </el-col>
          ...
          <div class="grid-content bg-purple-light">
            <div class="screenUnified">
              ...
              <div style="height:38px">
                <selects
                  :num="onehotelselect.levels"
                  :eventName="'hotellevel_in'"
                  @hotellevel_in="dataProcessing"
                ></selects>
              </div>
            </div>
          </div>
        ...
        <el-col :span="6">
          <div class="grid-content bg-purple">
            <div class="screenUnified">
              ...
              <div style="height:38px">
                <selects
                  :num="onehotelselect.types"
                  :eventName="'hoteltype_in'"
                  @hoteltype_in="dataProcessing"
                ></selects>
              </div>
            </div>
          </div>
        ...
        <el-col :span="6">
          <div class="grid-content bg-purple-light">
            <div class="screenUnified">
              ...
              <div style="height:38px">
                <selects
                  :num="onehotelselect.assets"
                  :eventName="'hotelasset_in'"
                  @hotelasset_in="dataProcessing"
                ></selects>
              </div>
            </div>
          </div>
        ...
        <el-col :span="6">
          <div class="grid-content bg-purple">
            <div class="screenUnified" style="border-right:0">
              ...
              <div style="height:38px">
                <selects2
                  :num="onehotelselect.brands"
                  :eventName="'hotelbrand_in'"
                  @hotelbrand_in="dataProcessing"
                ></selects2>
              </div>
            </div>
          </div>
                ...
                <div class="star">
                  <div class="starToStar">
                    <div class="starToStarUp">
                      <span class="el-icon-star-on" style="color: #ccc;" v-for="v in 5" :key="v"></span>
                    </div>
            ...
            <div class="grid-content bg-purple-light">
              <div class="onHotelListOut">
                <div class="onHotelListOutRight">
                  <a
                    :href="hrefDo(item.name)"
                    v-for="(item,index) in v.products"
                    :key="index"
                    target="_blank"
                  >
                    <div>
...
export default {
  ...
  mounted() {
    ...
    nuxt.onclick = e => {
      let numberOfPeopleChange = document.querySelector(
        ".numberOfPeopleChange"
      );
      if (
        e.target === numberOfPeopleChange ||
        e.target.offsetParent === numberOfPeopleChange
      )
        return;
  ...
  methods: {
      ...
      for (var i = 0, marker; i < lnglats.length; i++) {
        var marker = new AMap.Marker({
          ...
          content:
            ...
            `   <div class="close-btn" style="line-height: 25px;position: absolute;top: 0px;left: 0px;color: #fff;width: 100%;height: 100%;text-align: center;">${i +
              1}</div>` +
            "</div>"
        });
    ...
    getComplexHotel() {
      ...
      if (this.time) {
        parameter += `enterTime=${this.time[0].getFullYear()}-${this.time[0].getMonth() +
          1}-${this.time[0].getDate()}&`;
        parameter += `leftTime=${this.time[1].getFullYear()}-${this.time[1].getMonth() +
          1}-${this.time[1].getDate()}&`;
      }
